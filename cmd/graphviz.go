...
func main() {
    ...
    fmt.Println(bdd.Sat(algorithm.FromExpression(operators.Not(operators.Implies(p, operators.Cons(true))))))

    cnf := algorithm.FromExpression(operators.And(operators.Or(p, operators.Not(q)), operators.Or(q, r), operators.Or(q, s), operators.Or(p, operators.Not(s)), operators.Or(operators.Not(q), s), operators.Or(operators.Not(p), operators.Not(r))))
